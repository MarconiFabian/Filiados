...
export default function App() {
  ...
  const formatText = (p: Product) => {
    const lines = [
      ...
      ``,
      p.originalPrice ? `${globalSettings.showEmojiPriceOriginal ? globalSettings.emojiPriceOriginal + ' ' : ''}${getEffectiveLabel(p.labelOriginalPrice, globalSettings.labelOriginalPrice)} ~${p.originalPrice}~` : null,
      `${globalSettings.showEmojiPrice ? globalSettings.emojiPrice + ' ' : ''}${getEffectiveLabel(p.labelPrice, globalSettings.labelPrice)} *${p.price}*`,
                  ...
                  <div className="p-1 space-y-1 text-slate-800 leading-tight">
                    ...
                    {localProduct.originalPrice && (
                      <p className="text-[12px] text-slate-400">
                        ...
                        {getEffectiveLabel(localProduct.labelOriginalPrice, globalSettings.labelOriginalPrice)} <span className="line-through">{localProduct.originalPrice}</span>
                      </p>
                    )}
                    <div className="h-2" />
                    <p className="text-[13px]">
                      {globalSettings.showEmojiPrice ? globalSettings.emojiPrice + ' ' : ''}
                      {getEffectiveLabel(localProduct.labelPrice, globalSettings.labelPrice)} <span className="font-bold">{localProduct.price}</span>
                    </p>
                    {localProduct.coupon && (
                      <p className="text-[13px]">
