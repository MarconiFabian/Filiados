(() => {
  ...
  async function run() {
    ...
    const delay = Math.max(5, Number(state.mlDelaySeconds) || 30);
    let index = Math.max(0, Number(state.mlQueueIndex) || 0);
    if (!queue.length) { setStatus('A fila está vazia. Volte ao ML Afiliados Pro e envie novamente.', true); running = false; return; }
    ...
    if (!stopped && index >= queue.length) {
      setStatus('Concluído: todas as ofertas foram enviadas com foto.');
